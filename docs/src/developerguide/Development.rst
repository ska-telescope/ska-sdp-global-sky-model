...
The database schema is automatically generated by the ``scripts/generate_models.py``
...
3. Generate SQLAlchemy model classes in ``src/ska_sdp_global_sky_model/api/app/models.py``
4. Add appropriate column mappings, foreign keys, and helper methods

The generated ``models.py`` file is committed to the repository and should not be
manually edited. Always regenerate it using the ``make generate-models`` command.
...
After updating the schema (by regenerating models.py or modifying models),
