...
Revision ID: f2822a3a3662
Revises: 624b22e2fd6a
Create Date: 2026-02-04 12:32:09.562654
...
def upgrade() -> None:
    ...
    op.drop_table('band')
    op.add_column('source', sa.Column('healpix_index', sa.BigInteger(), nullable=False))
    op.add_column('source', sa.Column('component_id', sa.String(), nullable=True))
    ...
    op.drop_constraint(op.f('source_name_key'), 'source', type_='unique')
...
def downgrade() -> None:
    ...
    op.add_column('source', sa.Column('name', sa.VARCHAR(), autoincrement=False, nullable=True))
    ...
    op.create_unique_constraint(op.f('source_name_key'), 'source', ['name'], postgresql_nulls_not_distinct=False)
    ...
    op.drop_column('source', 'component_id')
